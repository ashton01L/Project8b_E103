...
def words_in_both(string1, string2):
    ...
    # Split the strings into lists of words and convert them to lower-case sets
    word_set1 = string1.lower().split()
    word_set2 = string2.lower().split()

    common_words = []

    # Iterate over the words in the word_set1 with a for loop
    for word in word_set1:
        # Cross check those words against word_set2 and to ensure they are not already in common_words
        if word in word_set2 and word not in common_words:
